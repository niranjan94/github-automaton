...
var githubLogin = function () {
  var output = childProcess.execSync(`curl -v -L ${ghProtocol}://${ghHost}/login 2>&1`, { encoding: 'utf8' }).toString().split('<!DOCTYPE html>');
...
var getGithubLoginResponseHeaders = function () {
  ...
  var commandArr = [`${ghProtocol}://${ghHost}/session`, `--silent`, `-v`, `-d`, `utf8=%E2%9C%93&authenticity_token=${authenticity_token}`, `-d`, `login=${USERNAME}`, `--data-urlencode`, `password=${PASSWORD}`, `-H`, `Pragma: no-cache`, `-H`, `Origin: ${ghProtocol}://${ghHost}`, `-H`, `Accept-Encoding: gzip, deflate`, `-H`, `Accept-Language: en-US,en;q=0.8`, `-H`, `Upgrade-Insecure-Requests: 1`, `-H`, `User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/46.0.2490.86 Safari/537.36`, `-H`, `Content-Type: application/x-www-form-urlencoded`, `-H`, `Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8`, `-H`, `Cache-Control: no-cache`, `-H`, `Referer: ${ghProtocol}://${ghHost}/`, `-H`, `Cookie: ${jar.get()}`, `-H`, `Connection: keep-alive`];

  return childProcess.spawnSync('curl', commandArr, { encoding: 'utf8' }).stderr.toString();
};
